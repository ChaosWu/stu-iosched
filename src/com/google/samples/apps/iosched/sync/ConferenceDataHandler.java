...
public class ConferenceDataHandler {
	...
	// Shared preferences key under which we store the timestamp that
	// corresponds to
	// the data we currently have in our content provider.
	private static final String SP_KEY_DATA_TIMESTAMP = "data_timestamp";

	// symbolic timestamp to use when we are missing timestamp data (which means
	// our data is
	...
	private static final String[] DATA_KEYS_IN_ORDER = { DATA_KEY_ROOMS,
			DATA_KEY_BLOCKS, DATA_KEY_TAGS, DATA_KEY_SPEAKERS,
			DATA_KEY_SESSIONS, DATA_KEY_SEARCH_SUGGESTIONS, DATA_KEY_MAP,
			DATA_KEY_HASHTAGS, DATA_KEY_EXPERTS, DATA_KEY_VIDEOS,
			DATA_KEY_PARTNERS };
	...
	// Tally of total content provider operations we carried out (for
	...
	/**
	 * Parses the conference data in the given objects and imports the data into
	 * the content provider. The format of the data is documented at
	 * https://code.google.com/p/iosched.
	 * 
	 * @param dataBodies
	 *            The collection of JSON objects to parse and import.
	 * @param dataTimestamp
	 *            The timestamp of the data. This should be in RFC1123 format.
	 * @param downloadsAllowed
	 *            Whether or not we are supposed to download data from the
	 *            internet if needed.
	 * @throws IOException
	...
	public void applyConferenceData(String[] dataBodies, String dataTimestamp,
		...
		LOGD(TAG, "Applying data from " + dataBodies.length
				+ " files, timestamp " + dataTimestamp);

		// create handlers for each data type
		mHandlerForKey.put(DATA_KEY_ROOMS, mRoomsHandler = new RoomsHandler(
				mContext));
		mHandlerForKey.put(DATA_KEY_BLOCKS, mBlocksHandler = new BlocksHandler(
				mContext));
		mHandlerForKey.put(DATA_KEY_TAGS, mTagsHandler = new TagsHandler(
				mContext));
		mHandlerForKey.put(DATA_KEY_SPEAKERS,
				mSpeakersHandler = new SpeakersHandler(mContext));
		mHandlerForKey.put(DATA_KEY_SESSIONS,
				mSessionsHandler = new SessionsHandler(mContext));
		mHandlerForKey.put(DATA_KEY_SEARCH_SUGGESTIONS,
				mSearchSuggestHandler = new SearchSuggestHandler(mContext));
		mHandlerForKey.put(DATA_KEY_MAP,
				mMapPropertyHandler = new MapPropertyHandler(mContext));
		mHandlerForKey.put(DATA_KEY_EXPERTS,
				mExpertsHandler = new ExpertsHandler(mContext));
		mHandlerForKey.put(DATA_KEY_HASHTAGS,
				mHashtagsHandler = new HashtagsHandler(mContext));
		mHandlerForKey.put(DATA_KEY_VIDEOS, mVideosHandler = new VideosHandler(
				mContext));
		mHandlerForKey.put(DATA_KEY_PARTNERS,
				mPartnersHandler = new PartnersHandler(mContext));

		// process the jsons. This will call each of the handlers when
		// appropriate to deal
		// with the objects we see in the data.
		LOGD(TAG, "Processing " + dataBodies.length + " JSON objects.");
		for (int i = 0; i < dataBodies.length; i++) {
			LOGD(TAG, "Processing json object #" + (i + 1) + " of "
					+ dataBodies.length);
			processDataBody(dataBodies[i]);
		}

		// the sessions handler needs to know the tag and speaker maps to
		// process sessions
		...
		processMapOverlayFiles(mMapPropertyHandler.getTileOverlays(),
				downloadsAllowed);
		...
		try {
			...
			if (operations > 0) {
				mContext.getContentResolver().applyBatch(
						ScheduleContract.CONTENT_AUTHORITY, batch);
			}
			LOGD(TAG, "Successfully applied " + operations
					+ " content provider operations.");
			mContentProviderOperationsDone += operations;
		} catch (RemoteException ex) {
			LOGE(TAG,
					"RemoteException while applying content provider operations.");
			throw new RuntimeException(
					"Error executing content provider batch operation", ex);
		} catch (OperationApplicationException ex) {
			LOGE(TAG,
					"OperationApplicationException while applying content provider operations.");
			throw new RuntimeException(
					"Error executing content provider batch operation", ex);
		}
		...
		LOGD(TAG,
				"Notifying changes on all top-level paths on Content Resolver.");
	...
	/**
	 * Processes a conference data body and calls the appropriate data type
	 * handlers to process each of the objects represented therein.
	 * 
	 * @param dataBody
	 *            The body of data to process
	 * @throws IOException
		...
		try {
			...
			while (reader.hasNext()) {
				...
				if (mHandlerForKey.containsKey(key)) {
					...
					LOGW(TAG, "Skipping unknown key in conference data json: "
							+ key);
					reader.skipValue();
				}
			}
	...
	/**
	 * Synchronise the map overlay files either from the local assets (if
	 * available) or from a remote url.
	 * 
	 * @param collection
	 *            Set of tiles containing a local filename and remote url.
	 * @throws IOException
	 */
	private void processMapOverlayFiles(Collection<Tile> collection,
			boolean downloadAllowed) throws IOException, SVGParseException {
		...
		for (Tile tile : collection) {
			...
			if (!MapUtils.hasTile(mContext, filename)) {
				...
				if (MapUtils.hasTileAsset(mContext, filename)) {
					...
					try {
						// download the file only if downloads are allowed and
						// url is not empty
						File tileFile = MapUtils
								.getTileFile(mContext, filename);
						...
						SVG svg = new SVGBuilder().readFromInputStream(is)
								.build();
						is.close();
	...
	public void setDataTimestamp(String timestamp) {
		...
		PreferenceManager.getDefaultSharedPreferences(mContext).edit()
				.putString(SP_KEY_DATA_TIMESTAMP, timestamp).commit();
	}
	...
	public static void resetDataTimestamp(final Context context) {
		LOGD(TAG,
				"Resetting data timestamp to default (to invalidate our synced data)");
		PreferenceManager.getDefaultSharedPreferences(context).edit()
				.remove(SP_KEY_DATA_TIMESTAMP).commit();
	}
