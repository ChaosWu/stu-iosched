 ...
 * indicate what is the list of sessions to show. It may be a set of tag filters
 * or a search query.
 * 
 * 一个{ @link ListFragment }显示会话列表。片段的参数显示会话列表中显示。它可能是一组标记过滤器或搜索查询
 */
public class SessionsFragment extends Fragment implements
	...
	// this variable is relevant when we start the sessions loader, and
	// indicates the desired
	// behavior when load finishes: if true, this is a full reload (for example,
	// because filters
	// have been changed); if not, it's just a refresh because data has changed.
	// 这个变量是相关的,当我们装载机启动会话,并显示所需的行为当加载完成:如果这是真的,
	// 这是一个完全重新加载(例如,因为过滤器改变);如果不是,它只是一个刷新,因为数据已经改变了。
	...
	private DateFormat mDateFormat = DateFormat
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == MESSAGE_QUERY_UPDATE) {
				...
				reloadFromArguments(BaseActivity
						.intentToFragmentArguments(new Intent(
								Intent.ACTION_SEARCH, ScheduleContract.Sessions
	...
	private void onSessionsContentChanged() {
		...
		LOGD(TAG,
				"Requesting sessions cursor reload as a result of ContentObserver firing.");
	...
	private void onTagsContentChanged() {
		...
		LOGD(TAG,
				"Requesting tags cursor reload as a result of ContentObserver firing.");
		reloadTagMetadata();
	}

	private void reloadSessionData(boolean fullReload) {
		LOGD(TAG, "Reloading session data: "
				+ (fullReload ? "FULL RELOAD" : "light refresh"));
		mSessionDataIsFullReload = fullReload;
		getLoaderManager().restartLoader(mSessionQueryToken, mArguments,
				SessionsFragment.this);
	}

	private void reloadTagMetadata() {
		getLoaderManager().restartLoader(TAG_METADATA_TOKEN, null,
				SessionsFragment.this);
	}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			...
			if (mArguments != null) {
				...
				mNoTrackBranding = mArguments
						.getBoolean(EXTRA_NO_TRACK_BRANDING);
			}

			if (mSessionQueryToken > 0) {
				// Only if this is a config change should we initLoader(), to
				// reconnect with an
	...
	private boolean useExpandedMode() {
		if (mCurrentUri != null
				&& ScheduleContract.Sessions.CONTENT_URI.equals(mCurrentUri)) {
	...
	void reloadFromArguments(Bundle arguments) {
		...
		// save arguments so we can reuse it when reloading from content
		// observer events
		...
		if (mCurrentUri == null) {
			...
			LOGD(TAG,
					"SessionsFragment did not get a URL, defaulting to all sessions.");
			arguments.putParcelable("_uri",
	...
	void requestQueryUpdate(String query) {
		...
		mHandler.sendMessageDelayed(
				Message.obtain(mHandler, MESSAGE_QUERY_UPDATE, query),
	...
	public void onAttach(Activity activity) {
		...
		mSessionsObserver = new ThrottledContentObserver(
		...
		mTagsObserver = new ThrottledContentObserver(
				new ThrottledContentObserver.Callbacks() {
					@Override
					public void onThrottledContentObserverFired() {
	...
	public void animateReload() {
		...
		// mCollectionView.animate().y(curTop).alpha(1).setDuration(ANIM_DURATION).setInterpolator(new
		// DecelerateInterpolator());
		mCollectionView.animate().alpha(1).setDuration(ANIM_DURATION)
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle data) {
		...
		if ((id == SessionsQuery.NORMAL_TOKEN || id == SessionsQuery.SEARCH_TOKEN)
				&& sessionsUri == null) {
			LOGD(TAG,
					"intent.getData() is null, setting to default sessions search");
			sessionsUri = ScheduleContract.Sessions.CONTENT_URI;
		}
		Loader<Cursor> loader = null;
		String liveStreamedOnlySelection = UIUtils
				.shouldShowLiveSessionsOnly(getActivity()) ? "IFNULL("
				+ ScheduleContract.Sessions.SESSION_LIVESTREAM_URL + ",'')!=''"
				: null;
		if (id == SessionsQuery.NORMAL_TOKEN) {
			LOGD(TAG, "Creating sessions loader for " + sessionsUri
					+ ", selection " + liveStreamedOnlySelection);
			loader = new CursorLoader(getActivity(), sessionsUri,
					SessionsQuery.NORMAL_PROJECTION, liveStreamedOnlySelection,
					null, ScheduleContract.Sessions.SORT_BY_TYPE_THEN_TIME);
		} else if (id == SessionsQuery.SEARCH_TOKEN) {
			LOGD(TAG, "Creating search loader for " + sessionsUri
					+ ", selection " + liveStreamedOnlySelection);
			loader = new CursorLoader(getActivity(), sessionsUri,
					SessionsQuery.SEARCH_PROJECTION, liveStreamedOnlySelection,
					null, ScheduleContract.Sessions.SORT_BY_TYPE_THEN_TIME);
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
		...
		LOGD(TAG, "Loader finished: "
				+ (token == SessionsQuery.NORMAL_TOKEN ? "sessions"
						: token == SessionsQuery.SEARCH_TOKEN ? "search"
								: token == TAG_METADATA_TOKEN ? "tags"
										: "unknown"));
		if (token == SessionsQuery.NORMAL_TOKEN
				|| token == SessionsQuery.SEARCH_TOKEN) {
			if (mCursor != null && mCursor != cursor) {
				mCursor.close();
			}
			mCursor = cursor;
			mIsSearchCursor = token == SessionsQuery.SEARCH_TOKEN;
			LOGD(TAG, "Cursor has " + mCursor.getCount()
					+ " items. Will now update collection view.");
	...
	private void updateCollectionView() {
		if (mCursor == null || mTagMetadata == null) {
			LOGD(TAG, "updateCollectionView: not ready yet... "
					+ (mCursor == null ? "no cursor." : "no tag metadata."));
			// not ready!
			return;
		}
		LOGD(TAG, "SessionsFragment updating CollectionView... "
				+ (mSessionDataIsFullReload ? "(FULL RELOAD)"
						: "(light refresh)"));
		...
		LOGD(TAG,
				"Updating CollectionView with inventory, # groups = "
						+ inv.getGroupCount() + " total items = "
	...
	private void showEmptyView() {
		final String searchQuery = ScheduleContract.Sessions
				.isSearchUri(mCurrentUri) ? ScheduleContract.Sessions
				.getSearchQuery(mCurrentUri) : null;

		if (mCurrentUri.equals(ScheduleContract.Sessions.CONTENT_URI)) {
			// if showing all sessions, the empty view should say "loading..."
			// because
			// the only reason we would have no sessions at all is if we are
			// currently
			// preparing the database from the bootstrap data, which should only
			// take a few
			// seconds.
			mEmptyView.setVisibility(View.GONE);
			mLoadingView.setVisibility(View.VISIBLE);
		} else if (ScheduleContract.Sessions
				.isUnscheduledSessionsInInterval(mCurrentUri)) {
			// Showing sessions in a given interval, so say
			// "No sessions in this time slot."
			...
			// Empty search query (for example, user hasn't started to type the
			// query yet),
			...
			// Showing sessions as a result of search or filter, so say
	...
	private CollectionView.Inventory prepareInventory() {
		...
		mCursor.moveToPosition(-1);
		int nextGroupId = HERO_GROUP_ID + 1000; // to avoid conflict with the
												// special hero group ID
		LOGD(TAG, "Total cursor data items: " + mCursor.getCount());
		int dataIndex = -1;
		final long now = UIUtils.getCurrentTime(mAppContext);
		final boolean conferenceEnded = TimeUtils
				.hasConferenceEnded(mAppContext);
		LOGD(TAG, "conferenceEnded=" + conferenceEnded);

		final boolean expandedMode = useExpandedMode();
		final int displayCols = getResources().getInteger(
				expandedMode ? R.integer.explore_2nd_level_grid_columns
						: R.integer.explore_1st_level_grid_columns);
		LOGD(TAG, "Using " + displayCols + " columns.");
		mPreloader.setDisplayCols(displayCols);

		while (mCursor.moveToNext()) {
			// For each data item, we decide what group it should appear under,
			// then
			// we add it to that group (and create the group if it doesn't exist
			// yet).

			long sessionEnd = mCursor.getLong(mCursor
					.getColumnIndex(ScheduleContract.Sessions.SESSION_END));
			...
			if (expandedMode) {
				String tags = mCursor
						.getString(mCursor
								.getColumnIndex(ScheduleContract.Sessions.SESSION_TAGS));
				TagMetadata.Tag groupTag = tags == null ? null : mTagMetadata
						.getSessionGroupTag(tags.split(","));
				if (groupTag != null) {
					groupLabel = groupTag.getName();
				} else {
					groupLabel = getString(R.string.others);
				}
				groupLabel += (showAsPast ? " ("
						+ getString(R.string.session_finished) + ")" : "");
			} else {
				groupLabel = showAsPast ? getString(R.string.ended_sessions)
						: "";
			}

			LOGV(TAG, "Data item #" + dataIndex + ", sessionEnd=" + sessionEnd
					+ ", groupLabel=" + groupLabel + " showAsPast="
					+ showAsPast);
			...
			if (!useExpandedMode() && !showAsPast && heroGroup == null) {
				...
				group = heroGroup = new CollectionView.InventoryGroup(
						HERO_GROUP_ID).setDisplayCols(1) // hero item spans all
															// columns
						.setShowHeader(false).setHeaderLabel("");
			} else {
				// "list" and "map" are just shorthand variables pointing to the
				// right list and map
				ArrayList<CollectionView.InventoryGroup> list = showAsPast ? pastGroups
						: futureGroups;
			...
			LOGV(TAG, "...adding to group '" + groupLabel
					+ "' with custom data index " + dataIndex);
		...
		// the first group doesn't need a header label, because it's the
		// "default group"
		// if (groups.size() > 0) {
	...
	public View newCollectionHeaderView(Context context, ViewGroup parent) {
		...
		return inflater.inflate(R.layout.list_item_explore_header, parent,
				false);
	}

	@Override
	public void bindCollectionHeaderView(Context context, View view,
			int groupId, String groupLabel) {
	...
	@Override
	public View newCollectionItemView(Context context, int groupId,
			ViewGroup parent) {
	...
	public void bindCollectionItemView(Context context, View view, int groupId,
		...
		if (mCursor == null || !mCursor.moveToPosition(dataIndex)) {
			LOGW(TAG, "Can't bind collection view item, dataIndex="
					+ dataIndex
					+ (mCursor == null ? ": cursor is null"
							: ": bad data index."));
			return;
		}
		...
		// first, read session info from cursor and put it in convenience
		// variables
		final String sessionTitle = mCursor.getString(SessionsQuery.TITLE);
		final String speakerNames = mCursor
				.getString(SessionsQuery.SPEAKER_NAMES);
		final String sessionAbstract = mCursor
				.getString(SessionsQuery.ABSTRACT);
		...
		sessionColor = sessionColor == 0 ? getResources().getColor(
				R.color.default_session_color) : sessionColor;
		int darkSessionColor = 0;
		final String snippet = mIsSearchCursor ? mCursor
				.getString(SessionsQuery.SNIPPET) : null;
		final Spannable styledSnippet = mIsSearchCursor ? buildStyledSnippet(snippet)
				: null;
		...
		// now let's compute a few pieces of information from the data, which we
		// will use
		// later to decide what to render where
		final boolean hasLivestream = !TextUtils.isEmpty(mCursor
				.getString(SessionsQuery.LIVESTREAM_URL));
		final long now = UIUtils.getCurrentTime(context);
		final boolean happeningNow = now >= sessionStart && now <= sessionEnd;

		// text that says "LIVE" if session is live, or empty if session is not
		// live
		final String liveNowText = hasLivestream ? " "
				+ UIUtils.getLiveBadgeText(context, sessionStart, sessionEnd)
				: "";
		...
		if (mNoTrackBranding) {
			sessionColor = getResources().getColor(
					R.color.no_track_branding_session_color);
		}

		darkSessionColor = UIUtils.scaleSessionColorToDefaultBG(sessionColor);

		ImageView photoView = (ImageView) view
				.findViewById(R.id.session_photo_colored);
		if (photoView != null) {
			if (!mPreloader.isDimensSet()) {
				...
				photoView.post(new Runnable() {
					@Override
					public void run() {
						mPreloader.setDimens(finalPhotoView.getWidth(),
								finalPhotoView.getHeight());
					}
				});
			}
			// colored
			photoView
					.setColorFilter(mNoTrackBranding ? new PorterDuffColorFilter(
							getResources()
									.getColor(
											R.color.no_track_branding_session_tile_overlay),
							PorterDuff.Mode.SRC_ATOP)
							: UIUtils
									.makeSessionImageScrimColorFilter(darkSessionColor));
		} else {
		...
		// background of the container must be the session color to avoid a
		// white flash
		...
		if (!TextUtils.isEmpty(photo)) {
			...
			// cleaning the (potentially) recycled photoView, in case this
		...
		// render subtitle into either the subtitle view, or the short subtitle
		// view, as available
		if (subtitleView != null) {
			subtitleView.setText(UIUtils.formatSessionSubtitle(sessionStart,
					sessionEnd, roomName, mBuffer, context) + liveNowText);
		} else if (shortSubtitleView != null) {
			shortSubtitleView.setText(UIUtils.formatSessionSubtitle(
					sessionStart, sessionEnd, roomName, mBuffer, context, true)
					+ liveNowText);
		}
		...
		if (categoryView != null) {
			...
			if (groupTag != null
					&& !Config.Tags.SESSIONS.equals(groupTag.getId())) {
		...
		// if we are in condensed mode and this card is the hero card (big card
		// at the top
		...
		if (!useExpandedMode() && groupId == HERO_GROUP_ID) {
			...
			// if this is the wide hero layout, show or hide the card or the
			// session abstract
			// view, as appropriate (they are mutually exclusive).
			final View cardContainer = view
					.findViewById(R.id.message_card_container_wide);
			final View abstractContainer = view
					.findViewById(R.id.session_abstract);
			if (cardContainer != null && abstractContainer != null) {
				cardContainer.setVisibility(cardShown ? View.VISIBLE
						: View.GONE);
				abstractContainer.setVisibility(cardShown ? View.GONE
						: View.VISIBLE);
				abstractContainer.setBackgroundColor(darkSessionColor);
			}
		}

		// if this session is live right now, display the "LIVE NOW" icon on top
		// of it
		View liveNowBadge = view.findViewById(R.id.live_now_badge);
		if (liveNowBadge != null) {
			liveNowBadge
					.setVisibility(happeningNow && hasLivestream ? View.VISIBLE
							: View.GONE);
		}
	...
	private boolean setupMessageCard(View hero) {
		MessageCardView card = (MessageCardView) hero
				.findViewById(R.id.message_card);
		if (card == null) {
	...
	private void setupLocalOrRemoteCard(final MessageCardView card) {
		...
		card.setButton(0, getString(R.string.attending_remotely),
				CARD_ANSWER_ATTENDING_REMOTELY, false, 0);
		card.setButton(1, getString(R.string.attending_in_person),
				CARD_ANSWER_ATTENDING_IN_PERSON, true, 0);
		...
		card.setListener(new MessageCardView.OnMessageCardButtonClicked() {
			...
			public void onMessageCardButtonClicked(final String tag) {
				final boolean inPerson = CARD_ANSWER_ATTENDING_IN_PERSON
						.equals(tag);
				card.dismiss(true);
				...
				if (activity != null) {
					Toast.makeText(
							activity,
							inPerson ? R.string.explore_attending_in_person_toast
									: R.string.explore_attending_remotely_toast,
	...
	private void setupWifiOfferCard(final MessageCardView card) {
		card.setText(getString(TimeUtils.hasConferenceStarted(getActivity()) ? R.string.question_setup_wifi_after_i_o_start
				: R.string.question_setup_wifi_before_i_o_start));
		card.setButton(0, getString(R.string.no_thanks), CARD_ANSWER_NO, false,
				0);
				...
				mHandler.postDelayed(new Runnable() {
					...
					public void run() {
						if (CARD_ANSWER_YES.equals(tag)) {
							WiFiUtils.showWiFiDialog(SessionsFragment.this
									.getActivity());
						} else {
					...
					public void run() {
						if (CARD_ANSWER_YES.equals(tag)) {
							Intent intent = new Intent(Intent.ACTION_VIEW, Uri
									.parse(Config.IO_EXTENDED_LINK));
							startActivity(intent);
						}
						PrefUtils
								.markDismissedIOExtendedCard(SessionsFragment.this
										.getActivity());
					}
				}, CARD_DISMISS_ACTION_DELAY);
			}
	...
	private class Preloader extends ListPreloader<String> {
		...
		protected List<String> getItems(int start, int end) {
			// Our start and end are rows, we need to adjust them into data
			// columns
	...
	private interface SessionsQuery {
		...
		String[] NORMAL_PROJECTION = { BaseColumns._ID,
		...
		String[] SEARCH_PROJECTION = { BaseColumns._ID,
				ScheduleContract.Sessions.SESSION_ID,
				ScheduleContract.Sessions.SESSION_TITLE,
				ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE,
				ScheduleContract.Sessions.SESSION_START,
