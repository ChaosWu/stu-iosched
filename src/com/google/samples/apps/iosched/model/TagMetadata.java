...
public class TagMetadata {
	...
	public static CursorLoader createCursorLoader(Context context) {
		return new CursorLoader(context, ScheduleContract.Tags.CONTENT_URI,
				TagsQuery.PROJECTION, null, null, null);
	}
	...
	public List<Tag> getTagsInCategory(String category) {
		return mTagsInCategory.containsKey(category) ? Collections
				.unmodifiableList(mTagsInCategory.get(category)) : null;
	}
	...
	public Tag getSessionGroupTag(String[] sessionTags) {
		...
		for (String tagId : sessionTags) {
			...
			if (tag != null
					&& Config.Tags.SESSION_GROUPING_TAG_CATEGORY.equals(tag
							.getCategory())
	...
	public static Comparator<Tag> TAG_DISPLAY_ORDER_COMPARATOR = new Comparator<Tag>() {
		...
		public int compare(Tag tag, Tag tag2) {
			if (!TextUtils.equals(tag.getCategory(), tag2.getCategory())) {
				return Config.Tags.CATEGORY_DISPLAY_ORDERS.get(tag
						.getCategory())
						- Config.Tags.CATEGORY_DISPLAY_ORDERS.get(tag2
								.getCategory());
	...
	static public class Tag implements Comparable<Tag> {
		...
		public Tag(String id, String name, String category,
				int orderInCategory, String _abstract, int color) {
			mId = id;
			mName = name;
			mCategory = category;
