...
/**
 * This helper handles the UI flow for signing in and authenticating an account.
 * It handles connecting to the Google+ API to fetch profile data (name, cover
 * photo, etc) and also getting the auth token for the necessary scopes. The
 * life of this object is tied to an Activity. Do not attempt to share it across
 * Activities, as unhappiness will result.
 */
public class LoginAndAuthHelper implements GoogleApiClient.ConnectionCallbacks,
	...
	// Auth scopes we need
	public static final String AUTH_SCOPES[] = { Scopes.PLUS_LOGIN,
	...
	// when sign-in *fails*, we will show the UI only once and set this flag to
	// false.
	...
	// The Activity this object is bound to (we use a weak ref to avoid context
	// leaks)
	WeakReference<Activity> mActivityRef;

	// Callbacks interface we invoke to notify the user of this class of useful
	// events
	...
	public void start() {
		...
		if (mResolving) {
			...
			LOGD(TAG,
					"Helper ignoring signal to start because we're resolving a failure.");
			return;
		}
		...
		if (mGoogleApiClient == null) {
			...
			mGoogleApiClient = builder.addApi(Plus.API)
					...
					.setAccountName(mAccountName).build();
		}
	...
	public void onConnected(Bundle bundle) {
		...
		if (!AccountUtils.hasPlusInfo(activity, mAccountName)) {
			LOGD(TAG, "We don't have Google+ info for " + mAccountName
					+ " yet, so loading.");
			PendingResult<People.LoadPeopleResult> result = Plus.PeopleApi
					.load(mGoogleApiClient, "me");
		...
		if (!AccountUtils.hasToken(activity, mAccountName)) {
			LOGD(TAG, "We don't have auth token for " + mAccountName
					+ " yet, so getting it.");
			mTokenTask = new GetTokenTask();
			mTokenTask.execute();
		} else {
	...
	public void onConnectionFailed(ConnectionResult connectionResult) {
		...
		if (connectionResult.hasResolution()) {
			if (sCanShowSignInUi) {
				LOGD(TAG,
						"onConnectionFailed, with resolution. Attempting to resolve.");
				...
				LOGD(TAG,
						"onConnectionFailed with resolution but sCanShowSignInUi==false.");
				reportAuthFailure();
			}
			return;
		}
	...
	public void onResult(People.LoadPeopleResult loadPeopleResult) {
		LOGD(TAG, "onPeopleLoaded, status="
				+ loadPeopleResult.getStatus().toString());
		if (loadPeopleResult.getStatus().isSuccess()) {
			...
			if (personBuffer != null && personBuffer.getCount() > 0) {
				...
				AccountUtils.setPlusProfileId(mAppContext, mAccountName,
						currentUser.getId());
				String imageUrl = currentUser.getImage().getUrl();
				if (imageUrl != null) {
					imageUrl = Uri.parse(imageUrl).buildUpon()
							.appendQueryParameter("sz", "256").build()
							.toString();
				}
				LOGD(TAG, "Saving plus image URL: " + imageUrl);
				AccountUtils.setPlusImageUrl(mAppContext, mAccountName,
						imageUrl);
				LOGD(TAG,
						"Saving plus display name: "
								+ currentUser.getDisplayName());
				AccountUtils.setPlusName(mAppContext, mAccountName,
						currentUser.getDisplayName());
				...
				if (cover != null) {
					...
					if (coverPhoto != null) {
						LOGD(TAG,
								"Saving plus cover URL: " + coverPhoto.getUrl());
						AccountUtils.setPlusCoverUrl(mAppContext, mAccountName,
								coverPhoto.getUrl());
					}
				} else {
	...
	/**
	 * Handles an Activity result. Call this from your Activity's
	 * onActivityResult().
	 */
	public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_AUTHENTICATE
			...
			LOGD(TAG, "onActivityResult, req=" + requestCode + ", result="
					+ resultCode);
			...
			if (resultCode == Activity.RESULT_OK) {
				if (mGoogleApiClient != null) {
					LOGD(TAG,
							"Since activity result was RESULT_OK, reconnecting client.");
					mGoogleApiClient.connect();
				} else {
					LOGD(TAG,
							"Activity result was RESULT_OK, but we have no client to reconnect.");
				}
				...
				LOGW(TAG,
						"Failed to recover from a login/auth failure, resultCode="
								+ resultCode);
			}
			return true;
		}
	...
	private void showRecoveryDialog(int statusCode) {
		...
		if (sCanShowAuthUi) {
			...
			final Dialog d = GooglePlayServicesUtil.getErrorDialog(statusCode,
					activity, REQUEST_RECOVER_FROM_PLAY_SERVICES_ERROR);
			d.show();
		} else {
			LOGD(TAG,
					"Not showing Play Services recovery dialog because sCanShowSignInUi==false.");
			reportAuthFailure();
		}
	}
	...
	private void showAuthRecoveryFlow(Intent intent) {
		...
		if (sCanShowAuthUi) {
			...
			LOGD(TAG,
					"Not showing auth recovery flow because sCanShowSignInUi==false.");
			reportAuthFailure();
		}
	}

	private void reportAuthSuccess(boolean newlyAuthenticated) {
		LOGD(TAG, "Auth success for account " + mAccountName
	...
	private class GetTokenTask extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			try {
				if (isCancelled()) {
					LOGD(TAG,
							"doInBackground: task cancelled, so giving up on auth.");
					return null;
				}

				LOGD(TAG, "Starting background auth for " + mAccountName);
				final String token = GoogleAuthUtil.getToken(mAppContext,
						mAccountName, AUTH_TOKEN_TYPE);

				// Save auth token.
				LOGD(TAG,
						"Saving token: "
								+ (token == null ? "(null)" : "(length "
										+ token.length() + ")")
								+ " for account " + mAccountName);
		...
		protected void onPostExecute(String token) {
			...
			if (isCancelled()) {
				...
				LOGD(TAG,
						"Activity not started, so not reporting auth success.");
			} else {
		...
		private void postShowRecoveryDialog(final int statusCode) {
			...
			LOGD(TAG, "Requesting display of recovery dialog for status code "
					+ statusCode);
			activity.runOnUiThread(new Runnable() {
				...
				public void run() {
					if (mStarted) {
						...
						LOGE(TAG,
								"Activity not started, so not showing recovery dialog.");
					}
				}
			});
		}
			...
			activity.runOnUiThread(new Runnable() {
				...
				public void run() {
					if (mStarted) {
						...
						LOGE(TAG,
								"Activity not started, so not showing auth recovery flow.");
					}
				}
			});
		}
