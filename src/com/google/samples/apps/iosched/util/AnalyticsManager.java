...
public class AnalyticsManager {
	...
	private final static String TAG = LogUtils
			.makeLogTag(AnalyticsManager.class);
	...
	private static boolean canSend() {
		...
		/**
		 * / /我们只能发送Google Analytics当所有下列条件是真实的:
		 * 
		 * 1。这个模块已经被初始化。
		 * 
		 * 2。用户已经接受了服务条款。
		...
		return sAppContext != null && mTracker != null
				&& PrefUtils.isTosAccepted(sAppContext)
	...
	public static void sendScreenView(String screenName) {
		if (canSend()) {
			...
			//屏幕视图不记录(分析残疾或没准备好)。
			LOGD(TAG,
					"Screen View NOT recorded (analytics disabled or not ready).");
		}
	}
	...
	public static void sendEvent(String category, String action, String label,
		...
		if (canSend()) {
			...
			LOGD(TAG,
					"Analytics event ignored (analytics disabled or not ready).");
		}
	}
	...
	public static synchronized void initializeAnalyticsTracker(Context context) {
		...
		if (mTracker == null) {
			...
			mTracker = GoogleAnalytics.getInstance(context).newTracker(
					useProfile);
		}
	}
}
